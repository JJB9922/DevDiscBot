...
async def send_humble_embed(embed_data, client):
    embed = discord.Embed(
    ...
    await client.get_channel(1207442996161814578).send(embed=embed)

async def check_and_send_new_bundles(client):
    filename = 'data/filtered_books.json'
    ...
    for bundle in difference_books:
        await send_humble_embed(bundle, client)
